...
class ThermalPrinter {
  constructor(initConfig) {
    ...
    if (initConfig.type === this.types.EPSON) {
  ...
  print(text) {
    text = text || "";
    if (this.config.type == 'other')
      this.append(iconv.encode(text, this.config.characterSet));
    else
  ...
  openCashDrawer() {
    ...
    else if (this.config.type == 'other') {
      // await this.raw(String.fromCharCode(27) + String.fromCharCode(112) + String.fromCharCode(0) + String.fromCharCode(25) + String.fromCharCode(250));
      this.raw(String.fromCharCode(27) + "=" + String.fromCharCode(1) + String.fromCharCode(27) + "p0{{");
    }
    else {
  ...
  table(data) {
    ...
    for (var i = 0; i < data.length; i++) {
      if (this.config.type == 'other')
				this.append(iconv.encode(data[i], this.config.characterSet));
			else
  ...
  tableCustom(data) {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      if (obj.align == "CENTER") {
        ...
        if (obj.text != '') {
					if (this.config.type == 'other')
						this.append(iconv.encode(obj.text, this.config.characterSet));
					else
						this.append(obj.text.toString());
        }
